...
- `freelancing-training/` — hub page (`index.html`) + 21 per-technology subpages, e.g. `linux-freelancing-training/`, `hermes-agent-training/`; replaces `/training/`
- `linux-ai-services/` — hub page + 23 per-technology subpages, all named `<tech>-services/` (e.g. `linux-server-services/`, `gitlab-services/`); replaces `/tech-support/`; renamed from `linux-it-services/` on 2026-07-16 (`.htaccess` 301-redirects the old slug, including subpaths)
